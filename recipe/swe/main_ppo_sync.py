...
# Modified by plm in 2026 for synchronous SWE agent-RL training and diagnostics.
#
...
from verl.experimental.agent_loop import (AgentLoopManager, AgentLoopOutput,
                                          AgentLoopWorker, get_trajectory_info)
...
from verl.trainer.ppo.utils import (Role, WorkerType, need_critic,
                                    need_reference_policy, need_teacher_policy)
...
from recipe.swe.env_server.container_gc import ContainerGCManager, ContainerGCSettings
...
def _pad_nested_or_keep_dense(value: torch.Tensor, padding: int) -> torch.Tensor:
    """Pad jagged TQ tensors while leaving already-dense batches unchanged.

    A batch made entirely from fixed-size fake trajectories is stacked into a
    regular CPU tensor.  Calling ``to_padded_tensor`` on that dense tensor is
    invalid, whereas mixed-length trajectories arrive as a NestedTensor and
    still require padding before decoding/dumping.
    """

    if value.is_nested:
        return torch.nested.to_padded_tensor(value, padding=padding)
    return value
...
class PPOTrainer:
    ...
    def _swe_diagnostics_config(self):
        return self.config.trainer.get("swe_step_diagnostics", {})

    def _swe_diagnostics_enabled(self) -> bool:
        return bool(self._swe_diagnostics_config().get("enable", False))

    @staticmethod
    def _diagnostic_records_from_extra_fields(extra_fields: Any) -> list[Any]:
        records = []
        for item in to_python_list(extra_fields):
            records.append(item.get("swe_diagnostics") if isinstance(item, dict) else None)
        return records

    def _emit_swe_step_diagnostics(
        self,
        records: list[Any],
        group_uids: list[Any],
        *,
        ...
        expected_group_uids: list[Any] | None = None,
    ) -> dict[str, float]:
        """Aggregate once on the PPO driver; never print from Ray workers."""

        if not self._swe_diagnostics_enabled():
            return {}
        cfg = self._swe_diagnostics_config()
        try:
            summary = aggregate_step_diagnostics(
                ...
                expected_group_uids=expected_group_uids,
                max_examples_per_outcome=int(cfg.get("max_examples_per_outcome", 3)),
            )
            swe_cfg = self.config.actor_rollout_ref.swe_custom_config
            multi_turn_cfg = self.config.actor_rollout_ref.rollout.multi_turn
            summary["limits"] = {
                "action_timeout_seconds": int(swe_cfg.get("action_execute_timeout", 0)),
                "action_timeout_limit": int(swe_cfg.get("max_env_timeout_cnt", 0)),
                "rollout_trajectory_timeout_seconds": int(
                    swe_cfg.get("max_rollout_trajectory_timeout", 0)
                ),
                "repeated_action_warning_threshold": int(
                    swe_cfg.get("repeated_action_warning_threshold", 0)
                ),
                "repeated_action_termination_threshold": int(
                    swe_cfg.get("repeated_action_termination_threshold", 0)
                ),
                "eval_timeout_seconds": int(swe_cfg.get("eval_timeout", 0)),
                "max_assistant_turns": int(multi_turn_cfg.get("max_assistant_turns", 0)),
                "max_response_length": int(self.config.data.get("max_response_length", 0)),
            }
        except Exception:
            # Diagnostics must never terminate a costly training run.
            logger.warning(
                "Failed to aggregate SWE step diagnostics for phase=%s step=%s",
                phase,
                self.global_steps,
                exc_info=True,
            )
        ...
        if bool(cfg.get("console", True)):
            try:
                logger.info("\n%s", format_step_diagnostics(summary))
            except Exception:
                logger.warning("Failed to render SWE step diagnostics", exc_info=True)
        if bool(cfg.get("write_json", True)):
            try:
                json_dir = cfg.get("json_dir", None)
                if not json_dir:
                    json_dir = os.path.join(self.config.trainer.default_local_dir, "step_diagnostics")
                path = write_step_diagnostics(summary, str(json_dir))
                logger.info("SWE step diagnostics written to %s", path)
            except Exception:
                logger.warning("Failed to write SWE step diagnostics JSON", exc_info=True)
        if not bool(cfg.get("tracker", True)):
            return {}
        try:
            return flatten_step_diagnostics(summary)
        except Exception:
            logger.warning("Failed to flatten SWE step diagnostics metrics", exc_info=True)
            return {}

    def _collect_train_swe_diagnostics(self, batch: KVBatchMeta) -> tuple[list[Any], list[Any]]:
        """Fetch final, non-padding trajectory diagnostics from TQ."""

        padding_flags = [bool(tag.get("is_padding", False)) for tag in batch.tags]
        final_indices = select_final_session_indices(batch.keys, padding_flags)
        final_keys = [batch.keys[index] for index in final_indices]
        if not final_keys:
            return [], []
        data = tq.kv_batch_get(
            keys=final_keys,
            partition_id=batch.partition_id,
            select_fields=["uid", "extra_fields"],
        )
        uids = to_python_list(data["uid"])
        records = self._diagnostic_records_from_extra_fields(data["extra_fields"])
        return records, uids
    ...
    def _validate(self) -> dict[str, float]:
        ...
        validation_diagnostic_records: list[Any] = []
        validation_diagnostic_uids: list[Any] = []
        validation_reports: list[Any] = []
        validation_expected_group_uids: list[Any] = []
        ...
        for batch_dict in self.val_dataloader:
            ...
            if self._swe_diagnostics_enabled():
                validation_expected_group_uids.extend(to_python_list(batch_dict["uid"]))
            ...
            text_data["prompts"] = _pad_nested_or_keep_dense(
                text_data["prompts"], self.tokenizer.pad_token_id
            )
            text_data["responses"] = _pad_nested_or_keep_dense(
                text_data["responses"], self.tokenizer.pad_token_id
            )
            ...
            fields = ["uid", "rm_scores", "num_turns", "reward_model", "data_source", "extra_fields", "extra_info"]
            data = tq.kv_batch_get(keys=final_keys, partition_id=batch.partition_id, select_fields=fields)

            batch_sample_uids = to_python_list(data.pop("uid"))
            sample_uids.extend(batch_sample_uids)
            sample_outputs.extend(all_outputs[i] for i in final_indices)
            # Preserve task metadata, evaluation reports, and step diagnostics.
            extra_info_list = data.pop("extra_info", None)
            extra_fields_list = data.get("extra_fields", None)

            extra_infos = to_python_list(extra_info_list) if extra_info_list is not None else [None] * len(final_indices)
            extra_fields_items = (
                to_python_list(extra_fields_list) if extra_fields_list is not None else [None] * len(final_indices)
            )

            for i, idx in enumerate(final_indices):
                input_text = all_inputs[idx]
                saved = {"text": input_text}

                # Check and save extra_info
                info_item = extra_infos[i]
                if isinstance(info_item, dict):
                    saved["extra_info"] = info_item

                # Check and save report
                fields_item = extra_fields_items[i]
                if isinstance(fields_item, dict) and "report" in fields_item:
                    saved["report"] = fields_item["report"]
                if isinstance(fields_item, dict) and "swe_diagnostics" in fields_item:
                    saved["swe_diagnostics"] = fields_item["swe_diagnostics"]
                sample_inputs.append(saved)
                if self._swe_diagnostics_enabled():
                    validation_diagnostic_records.append(
                        fields_item.get("swe_diagnostics") if isinstance(fields_item, dict) else None
                    )
                    validation_diagnostic_uids.append(batch_sample_uids[i])
                    validation_reports.append(
                        fields_item.get("report") if isinstance(fields_item, dict) else None
                    )
            ...
            if extra_fields_list is not None:
                extra_fields_items = to_python_list(extra_fields_list)
                n_prior = len(reward_extra_infos_dict["reward"]) - len(extra_fields_items)
                for extra_field in extra_fields_items:
                    reward_extra_info = (
        ...
        if val_data_dir:
            ...
            dump_reward_extra_infos = {
                k: [v[i] for i in session_final_indices] for k, v in reward_extra_infos_dict.items()
            } | {"uid": dump_all_keys}
            if self._swe_diagnostics_enabled():
                dump_reward_extra_infos["report"] = [
                    validation_reports[i] for i in session_final_indices
                ]
                dump_reward_extra_infos["swe_diagnostics"] = [
                    validation_diagnostic_records[i] for i in session_final_indices
                ]
        ...
        if self._swe_diagnostics_enabled():
            val_metrics.update(
                self._emit_swe_step_diagnostics(
                    validation_diagnostic_records,
                    validation_diagnostic_uids,
                    phase="validation",
                    expected_group_size=int(self.config.actor_rollout_ref.rollout.val_kwargs.n),
                    expected_group_uids=validation_expected_group_uids,
                )
            )
        return val_metrics
    ...
    def _maybe_log_val_generations(self, inputs, outputs, scores):
        ...
        samples.sort(
            key=lambda x: str(x[0].get("text", "")) if isinstance(x[0], dict) else str(x[0])
        )
    ...
    def _log_rollout_data(self, batch: KVBatchMeta, timing_raw: dict, rollout_data_dir: str):
        ...
        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
            fields = ["uid", "prompts", "responses", "rm_scores", "reward_model", "extra_info", "extra_fields"]
            data = tq.kv_batch_get(keys=batch.keys, partition_id=batch.partition_id, select_fields=fields)
            data["prompts"] = _pad_nested_or_keep_dense(
                data["prompts"], self.tokenizer.pad_token_id
            )
            data["responses"] = _pad_nested_or_keep_dense(
                data["responses"], self.tokenizer.pad_token_id
            )

            uids = data.pop("uid").tolist()
            # Preserve task metadata, evaluation reports, and step diagnostics.
            input_texts = self.tokenizer.batch_decode(data["prompts"], skip_special_tokens=True)
            inputs = []

            extra_info_list = data.pop("extra_info", None)
            extra_fields_list = data.pop("extra_fields", None)

            extra_infos = to_python_list(extra_info_list) if extra_info_list is not None else [None] * len(input_texts)
            extra_fields_items = (
                to_python_list(extra_fields_list) if extra_fields_list is not None else [None] * len(input_texts)
            )
    ...
    def _compute_metrics(self, batch: KVBatchMeta, metrics, timing_raw, global_steps, epoch):
        diagnostic_records: list[Any] = []
        diagnostic_uids: list[Any] = []
        if self._swe_diagnostics_enabled():
            diagnostic_records, diagnostic_uids = self._collect_train_swe_diagnostics(batch)
        ...
        if self._swe_diagnostics_enabled():
            metrics.update(
                self._emit_swe_step_diagnostics(
                    ...
                    expected_group_uids=getattr(self, "_swe_expected_train_uids", None),
                )
            )
    ...
    def fit(self):
        ...
        if self.config.trainer.get("val_before_train", True):
            ...
            assert any(not str(key).startswith("swe_diag/") for key in val_metrics), f"{val_metrics=}"
    ...
    def step(self, batch_dict: dict, metrics: dict, timing_raw: dict) -> KVBatchMeta:
        ...
        if self._swe_diagnostics_enabled():
            self._swe_expected_train_uids = to_python_list(batch_dict["uid"])
        if self.config.algorithm.adv_estimator == core_algos.AdvantageEstimator.REMAX:
...
class TaskRunner:
    ...
    def run(self, config):
        ...
        trainer = None
        container_gc_manager = None
        try:
            container_gc_settings = ContainerGCSettings.from_mapping(
                config.actor_rollout_ref.swe_custom_config
            )
            container_gc_manager = ContainerGCManager.start(
                settings=container_gc_settings,
                expected_groups=int(config.trainer.nnodes),
            )
            ...
            try:
                ...
                try:
                    if container_gc_manager is not None:
                        gc_reports = container_gc_manager.drain_and_shutdown()
                        not_drained = [
                            {"group_id": group_id, **report}
                            for group_id, report in gc_reports.items()
                            if not report.get("drained", False)
                        ]
                        if not_drained:
                            logger.warning(
                                "Container GC shutdown left queued work for TTL fallback: %s",
                                not_drained,
                            )
                finally:
                    tq.close()
